...
function Assert-DirectImageTopology {
    ...
    $walletFiles = @($managed | Where-Object { $_.Type -eq "local_sensitive_file" })
    ...
    if ($autonomousDatabases.Count -ne 1 -or $wallets.Count -ne 1 -or $walletFiles.Count -ne 1) {
        throw "$Label must contain exactly one ADB, ADB wallet, and local protected wallet file."
    }
    if ($autonomousDatabases[0].Address -ne "module.adb.oci_database_autonomous_database.pilot" -or
        $wallets[0].Address -ne "module.adb.oci_database_autonomous_database_wallet.pilot" -or
        $walletFiles[0].Address -ne "module.adb.local_sensitive_file.wallet") {
        throw "$Label contains an unapproved ADB wallet resource address."
    }
...
function Install-RuntimeFiles {
    ...
    foreach ($file in $files) {
        ...
        if ([string]::IsNullOrWhiteSpace($extractTo)) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($containerUid)) {
                ...
                $postRestartCommands += "$waitForContainer && sleep 3 && $containerAcl && $containerReadCheck && sleep 2 && $containerReadCheck"
            }
    ...
    $runtimeCommands += " && systemctl --user restart oci-image-pilot.service"
...
function Invoke-RemoteVerification {
    ...
    # For a oneshot unit, "active" is reached only after ExecStartPost has
    # restored protected bind-mount ACLs. This prevents the reboot verifier
    # from racing the persistent wallet-access hook.
    $remoteCommand = "bash -lc 'for attempt in {1..90}; do if systemctl --user is-active --quiet oci-image-pilot.service; then exec /home/opc/oci-image-pilot/tests/run-tests.sh --wait $TimeoutSeconds --expect-source oci; fi; sleep 2; done; exit 1'"
    $arguments = Get-SshArguments `
        -PrivateKeyPath $PrivateKeyPath `
        -KnownHostsPath $KnownHostsPath `
        -Target $Target `
...
function Invoke-RemoteBaselineToolVerification {
    ...
    # SQLcl is an operator tool. Verify its installed launcher and the two JDK
    # packages without trying to start an interactive client during acceptance.
    $remoteCommand = 'command -v sql >/dev/null 2>&1 && test -x /usr/local/bin/sql && test -x /opt/sqlcl/bin/sql && rpm -q jdk-21-headless >/dev/null 2>&1 && rpm -q jdk-26-headless >/dev/null 2>&1'
    $arguments = Get-SshArguments `
        -PrivateKeyPath $PrivateKeyPath `
        -KnownHostsPath $KnownHostsPath `
        -Target $Target `
    ...
    try {
        ...
        throw "The test VM does not meet the required SQLcl and JDK 26 baseline: $($_.Exception.Message)"
    }
    Write-Pass "SQLcl launcher plus JDK 21 and JDK 26 baseline verified on the test VM"
}
