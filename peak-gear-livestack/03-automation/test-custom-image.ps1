...
$liveStackImageBuildDirectory = Join-Path $workspaceRoot "livestack\ailakehouse\_deploy\auto_build\01-image-build"
$liveStackImageBuildDirectory = Resolve-ExistingDirectory `
    -Path $liveStackImageBuildDirectory `
    -Label "AILakehouse image-build directory"
if ([string]::IsNullOrWhiteSpace($PublicEndpointsFile)) {
    $PublicEndpointsFile = Join-Path $liveStackImageBuildDirectory "01-edit\public-endpoints.json"
}
$VariableFile = Resolve-ExistingFile -Path $VariableFile -Label "Terraform variable file"
...
try {
    ...
    try {
        ...
        if ($InspectionMode) {
            ...
            $verificationHarnessPath = Join-Path $liveStackImageBuildDirectory "03-automation\run-tests.sh"
            $serviceTestsPath = Join-Path $liveStackImageBuildDirectory "02-edit-if-needed\service-tests"
