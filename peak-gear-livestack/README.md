...
This Terraform Play project is called by
`livestack/ailakehouse/_deploy/auto_build/01-image-build`. Keep
`Terraform-Repo-Oracle` and `livestack` beside each other, or pass the LiveStack
launcher an explicit `-TerraformDirectory` path. It is not normally run directly.
