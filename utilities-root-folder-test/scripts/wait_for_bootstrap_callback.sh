...
log "Waiting up to $(( WAIT_TIMEOUT_SECONDS / 60 )) minutes for the Utilities bootstrap callback."
...
while (( $(date +%s) < deadline )); do
  ...
  if [[ -n "${status}" ]]; then
    status_format="$(status_field format "${status}")"
    ...
    exit_code="$(status_field exit_code "${status}")"
    error_class="$(status_field error_class "${status}")"
    error_code="$(status_field error_code "${status}")"
    error_step="$(status_field error_step "${status}")"
    ...
    if [[
      "${status_format}" != "utilities-rm-bootstrap/v1" ||
        "${deployment_id}" != "${EXPECTED_DEPLOYMENT_ID}" ||
        "${instance_ocid}" != "${EXPECTED_INSTANCE_OCID}"
    ]]; then
      if [[
        "${status_format}" == "utilities-rm-bootstrap/v1" &&
        "${state}" == "PENDING" &&
    ...
    signature="${status_format}|${state}|${phase}|${marker}"
    ...
    case "${state}" in
      SUCCEEDED)
        if [[ "${marker}" == "RESOURCE_MANAGER_DEPLOYMENT_OK" ]]; then
          log "Utilities bootstrap verified successfully; native Select AI acceptance and the application health check passed."
          exit 0
        fi
      ...
      FAILED)
        ...
        if [[ "${error_class}" =~ ^[a-z0-9_]{1,96}$ ]]; then
          log "Bootstrap error class: ${error_class}."
        fi
        if [[ "${error_code}" =~ ^(ORA|PLS|SP2)-[0-9]{4,5}$ ]]; then
          log "Bootstrap error code: ${error_code}."
        fi
        if [[ "${error_step}" =~ ^[A-Za-z0-9_.-]{1,128}$ ]]; then
          log "Bootstrap SQL step: ${error_step}."
        fi
        if [[ "${exit_code}" =~ ^[0-9]{1,3}$ ]]; then
...
log "Timed out waiting for Utilities bootstrap completion."
log "Use bootstrap_log_command in the stack outputs for the detailed VM log."
exit 1
