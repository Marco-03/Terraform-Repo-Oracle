...
locals {
  ...
  application_payload_path   = abspath("${path.module}/payload/utilities-application.zip")
...
locals {
  ...
  application_name = "utilities"
  application_user = "APP_USER"
  application_port = 8505

  select_ai_profile            = "UTILITIES_SELECTAI_V1"
  select_ai_primary_agent_team = "UTILITIES_OPERATIONS_TEAM"
  select_ai_agent_teams        = "UTILITIES_OPERATIONS_TEAM,OUTAGE_RESTORATION_TEAM,FIELD_OPERATIONS_TEAM"

  vcn_cidr        = "10.44.0.0/16"
  app_subnet_cidr = "10.44.10.0/24"
  ...
  adb_database_name           = upper("UTL${random_id.deployment.hex}")
