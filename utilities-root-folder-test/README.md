# Utilities LiveStack: Native Select AI on OCI

This rootless Terraform bundle deploys the Utilities LiveStack through OCI
...
- ADB 26ai was created and loaded with the Utilities schema and data.
- An APP_USER Select AI profile was created over 11 curated, read-only
  semantic views.
- Three enabled native advisory teams, four agents, and three tool-free tasks
  were created, plus a separate bootstrap-only direct-validation SQL tool.
...
The default ADB size is two ECPUs to keep the demo comparatively affordable.
That size does **not** provide the Autonomous Database In-Memory column store;
Oracle documents a minimum of 16 ECPUs for that feature. The schema retains
the four table declarations and the UI reports them as declaration evidence
only. `DECLARED` never means populated or active. If you choose 16 or more
ECPUs, runtime population is still shown as unavailable unless ADB exposes
positive service-managed evidence; the application does not infer it from
DDL. See [Use Database In-Memory with Autonomous
Database](https://docs.oracle.com/en-us/iaas/autonomous-database-shared/doc/autonomous-in-memory.html).
...
- One VCN, internet gateway, public subnet, route table, and Network Security
  Groups.
- One Oracle Linux 9 Compute instance running the Utilities application in
  Podman.
...
- APP_USER, the Utilities schema and data, `UTILITIES_SELECTAI_V1`,
  three enabled teams, four agents, three tool-free runtime tasks, and
  `UTILITIES_READONLY_SQL_TOOL` used only by bootstrap.
...
1. The VM generates an unencrypted 2048-bit RSA key under
   `/opt/utilities-livestack` with mode `0600`.
...
1. Install Podman, SQLcl, Java, OpenSSL, and supporting utilities.
2. Generate and register the stack-specific API signing key.
3. Download the bundled application and generated ADB wallet.
4. Create APP_USER, load the Utilities schema/data, and run database
   checks.
5. Create the APP_USER OCI credential and `UTILITIES_SELECTAI_V1`.
6. Create the 11-view Select AI semantic scope, three enabled teams, four
   agents, three tool-free native advisory tasks, and the bootstrap-only
   `UTILITIES_READONLY_SQL_TOOL`.
...
8. Build/start the app and require HTTP 200 from `/api/health`.
...
```text
UTILITIES_BOOTSTRAP_COMPLETE
UTILITIES_HANDOFF_LOADER_COMPLETE
UTILITIES_NATIVE_AI_CHAT_OK
UTILITIES_NATIVE_AI_SHOWSQL_OK
UTILITIES_NATIVE_AI_RUNSQL_OK
UTILITIES_NATIVE_AI_AGENT_TASKS_TOOL_FREE_OK
UTILITIES_NATIVE_AI_AGENT_TOOL_OK
UTILITIES_NATIVE_AI_AGENT_OK
UTILITIES_NATIVE_AI_ACCEPTANCE_OK
UTILITIES_ADB_ACCEPTANCE_OK
RESOURCE_MANAGER_DEPLOYMENT_OK
```

Any failed package call, unsafe generated query, missing native object, failed
native tool/team descriptor check, invalid schema object, failed app build, or
failed health check prevents Apply from succeeding. No local-model or
deterministic AI fallback is used.
...
registered `UTILITIES_READONLY_SQL_TOOL` remains governed by the 11-view
profile and is used only for direct bootstrap acceptance through `RUN_TOOL`.
The application can record the returned recommendation as a proposed audit
entry; the agent cannot change Utilities business data or claim that an
action ran.
...
the app. Dataset import/restore execution requires the existing explicit
same-origin dataset-command guard and an admin demo identity, but those demo
controls are not production authentication. Treat `app_ingress_cidr` as the
access-control boundary, keep it limited to trusted operators, and never use
this deployment for production or sensitive data.
...
| `adb_license_model`, `adb_compute_count`, `adb_storage_tbs` | ADB licensing and sizing. The 2-ECPU default is below the 16-ECPU minimum for ADB In-Memory; declaration evidence is reported separately. |
| `adb_admin_password` | Optional ADMIN password; leave blank to generate it. |
| `model_object_uri` | Sensitive prefilled one-object HTTPS/PAR URI used by existing Utilities vector features; replace it with an approved URI if required. |
...
1. In **Ask Utilities Data**, ask “Which open work orders have the
   greatest production risk?” in Show SQL mode. Confirm that generated SQL
   references a curated `UTILITIES_*_V` view.
2. Run the same question in Run SQL or narrative mode. Confirm rows, model,
   region, action, and read-only evidence are shown.
3. Switch demo users and run a work-order or production-signal question.
   Confirm the visible data follows the user's VPD scope.
4. In **Agent Console**, run `UTILITIES_OPERATIONS_TEAM`. Confirm the
   response identifies `DBMS_CLOUD_AI_AGENT`, `RUN_TEAM`, the selected
   region/model, and advisory/read-only status.
...
Utilities business data. A successful run ends with
`UTILITIES_API_REGRESSION_OK`.
...
`dist/utilities-livestack-terraform.zip`:
...
`payload/utilities-application.zip`. Terraform references this already-present
...
python3 verification/check-resource-manager-package.py \
  --source-root . \
  --archive dist/utilities-livestack-terraform.zip
bash -n scripts/*.sh application/deployment/*.sh
node --check verification/utilities-livestack-regression.mjs
...
python3 verification/check-resource-manager-package.py \
  --source-root . \
  --archive /path/to/utilities-livestack-terraform.zip
bash -n scripts/*.sh
node --check verification/utilities-livestack-regression.mjs
```

To rerun application tests from that extracted archive, unzip
`payload/utilities-application.zip` into a temporary directory and run
