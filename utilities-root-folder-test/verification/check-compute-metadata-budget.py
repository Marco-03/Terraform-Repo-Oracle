...
def render_cloud_init() -> str:
    ...
    values = {
        **par_values,
        ...
        "adb_service_name_b64": encode("mfg12345678_high"),
        ...
        "select_ai_profile_b64": encode("UTILITIES_SELECTAI_V1"),
        "select_ai_primary_team_b64": encode(
            "UTILITIES_OPERATIONS_TEAM"
        ),
        "select_ai_agent_teams_b64": encode(
            "UTILITIES_OPERATIONS_TEAM,PRODUCTION_SIGNAL_TEAM,"
            "FULFILLMENT_TEAM"
        ),
...
def assert_terraform_contract() -> None:
    ...
    network_text = (SOURCE_ROOT / "network.tf").read_text(encoding="utf-8")
    outputs_text = (SOURCE_ROOT / "outputs.tf").read_text(encoding="utf-8")
    ...
    required_compute_fragments = (
        ...
        "ssh_enabled",
        "application_instance_metadata = merge(local.ssh_enabled ? {",
        ...
        "Provide both the SSH public key and SSH source CIDR, or leave both blank to disable SSH.",
    ...
    if not re.search(
        r'ssh_enabled\s*=\s*trimspace\(var\.ssh_public_key\) != "" && '
        r'trimspace\(var\.ssh_ingress_cidr\) != ""',
        compute_text,
    ):
        fail("compute-app.tf does not enable SSH only when both SSH values are supplied")
    ...
    if not re.search(r'ssh_public_key:.*?required:\s*false', schema_text, re.S):
        fail("Resource Manager schema still requires an SSH public key")
    if not re.search(r'ssh_ingress_cidr:.*?required:\s*false', schema_text, re.S):
        fail("Resource Manager schema still requires an SSH source CIDR")
    if 'default     = ""' not in variables_text:
        fail("Terraform does not permit blank SSH settings")
    if "count                     = local.ssh_enabled ? 1 : 0" not in network_text:
        fail("SSH ingress is not disabled when SSH is not configured")
    if 'SSH access was not configured for this stack.' not in outputs_text:
        fail("SSH output does not explain when SSH is disabled")
