...
variable "ssh_public_key" {
  description = "Optional OpenSSH public key used for opc access to the LiveStack VM. Leave blank to disable SSH access."
  type        = string
  default     = ""
  ...
  validation {
    ...
    error_message = "SSH public key must be blank or a valid OpenSSH public key no longer than 4,096 characters."
  }
}

variable "ssh_ingress_cidr" {
  description = "Optional public IPv4 CIDR (/24 to /32) allowed to SSH to the VM. Leave blank to disable SSH access."
  type        = string
  default     = ""
  ...
  validation {
    ...
    error_message = "SSH source CIDR must be blank or a valid IPv4 /24 to /32. Prefer the public IPv4 address of the computer, VPN, or bastion you will SSH from followed by /32."
  }
}

variable "app_ingress_cidr" {
  description = "Trusted IPv4 CIDR (/24 to /32) allowed to open the unauthenticated Utilities demo. For Internet clients, use the public egress IPv4 plus /32; an RFC1918 address works only through connected private networking."
  type        = string
...
variable "vm_shape" {
  description = "Compute shape for the single VM that runs the Utilities application container."
  type        = string
...
variable "adb_compute_count" {
  description = "ECPUs assigned to Autonomous Database. The affordable default is 2; ADB In-Memory column-store availability requires at least 16 ECPUs, and table declarations alone are not runtime-population proof."
  type        = number
  default     = 2
