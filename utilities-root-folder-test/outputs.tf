output "application_url" {
  description = "Public Utilities LiveStack URL, including port 8505. Apply succeeds only after the app is ready."
  value       = "http://${oci_core_instance.application.public_ip}:${local.application_port}/"
}

output "next_step" {
  description = "How to open the Utilities application."
...
output "vm_instance_ip" {
  description = "Raw public IP address of the application VM. Use application_url to open the Utilities application."
...
output "first_boot_status_command" {
  ...
  value       = "ssh opc@${oci_core_instance.application.public_ip} 'sudo cloud-init status --wait; result=$?; sudo cat /opt/utilities-livestack/deployment-status.txt 2>/dev/null || true; exit $result'"
}

output "bootstrap_log_command" {
  description = "Display the application, database, and native Select AI bootstrap log after an Apply failure."
  value       = "ssh opc@${oci_core_instance.application.public_ip} 'sudo tail -n 250 /var/log/utilities-livestack-bootstrap.log'"
}
...
output "application_database_user" {
  description = "Dedicated schema used by the Utilities application."
