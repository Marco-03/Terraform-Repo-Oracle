...
locals {
  ...
  selectai_api_key_description = "Utilities LiveStack ${var.stack_name} ${local.selectai_api_key_owner_token}"
}
...
resource "oci_identity_domains_my_api_key" "select_ai" {
  ...
  lifecycle {
    ...
    precondition {
      condition = try(
        local.selectai_key_callback.format == "utilities-selectai-api-key-public/v1" &&
